...
export const DesertCampsPage: React.FC = () => {
 ...
 {
 id: 1,
 title: t('dailyRent.mockTitle3', 'مخيم الشتاء الفاخر بتجهيزات متكاملة'),
 location: t('dailyRent.mockLoc3', 'الرياض، الثمامة'),
 price: '1,500',
 rentPeriod: t('dailyRent.daily', 'يومياً'),
 beds: 3,
 baths: 2,
 area: '1000 ' + t('citySale.sqm', 'م²'),
 isPremium: true,
 features: [t('dailyRent.feat9', 'خيام مكيفة'), t('dailyRent.feat10', 'دبابات'), t('dailyRent.feat8', 'مشب نار'), t('dailyRent.feat11', 'ملاعب طائرة')],
 image: 'https://images.unsplash.com/photo-1682688759157-57988e10ffa8?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxkZXNlcnQlMjBjYW1wJTIwdGVudCUyMGx1eHVyeSUyMHNhdWRpfGVufDF8fHx8MTc3MjczNTE2M3ww&ixlib=rb-4.1.0&q=80&w=1080',
 agent: { name: t('dailyRent.agent3', 'مخيمات العاصمة'), time: t('citySale.time1', 'منذ ساعتين'), logo: 'RC' }
 },
 {
 id: 2,
 title: t('desertCamps.mockTitle2', 'مخيم تراثي للعوائل فقط'),
 location: t('desertCamps.mockLoc2', 'الرياض، رماح'),
 price: '800',
 rentPeriod: t('dailyRent.daily', 'يومياً'),
 beds: 2,
 baths: 2,
 area: '800 ' + t('citySale.sqm', 'م²'),
 isPremium: false,
 features: [t('desertCamps.feat1', 'جلسات تراثية'), t('desertCamps.feat2', 'مطبخ متكامل'), t('desertCamps.feat3', 'قسمين منفصلين')],
 image: 'https://images.unsplash.com/photo-1725909632786-c25c9b2b5a16?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxiZWRvdWluJTIwdGVudCUyMGRlc2VydHxlbnwxfHx8fDE3NzI3MzUxOTZ8MA&ixlib=rb-4.1.0&q=80&w=1080',
 agent: { name: t('desertCamps.agent2', 'أجواء الشتاء'), time: t('citySale.time2', 'منذ يوم'), logo: 'WW' }
 }
 ];
 ...
 <span>{t('dailyRent.dailyRentInclusive', 'إيجار يومي')}</span>
 <span className="text-gray-300">/</span>
 <span className="font-bold text-gray-900">{t('desertCamps.title', 'مخيمات برية')}</span>
 </div>

 <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
 <div>
 <h1 className="text-3xl font-bold text-[#2B3D50] mb-2">{t('desertCamps.title', 'مخيمات برية')}</h1>
 <p className="text-gray-500">{t('desertCamps.foundProperties', 'تم العثور على {{count}} مخيم متاح', { count: properties.length })}</p>
 </div>
 ...
 <input type="text" placeholder={t('desertCamps.searchPlaceholder', 'ابحث بالمنطقة، اسم المخيم...')} className="w-full ps-4 pe-12 py-3.5 rounded-xl border border-gray-200 focus:outline-none focus:border-[#47CCD0] focus:ring-1 focus:ring-[#47CCD0] text-sm bg-gray-50/50" />
 <Search size={20} className="absolute end-4 top-1/2 -translate-y-1/2 text-gray-400" />
 </div>
 <FilterBar typeOptions={[t('dailyRent.camp', 'مخيم'), t('dailyRent.chalet', 'شاليه'), t('desertCamps.farm', 'مزرعة')]} hidePricePeriod={true} />
 </div>
 <div className="flex flex-wrap gap-2 pt-2">
 {[t('desertCamps.tag1', 'قسمين منفصلين'), t('desertCamps.tag2', 'مناسب للعوائل'), t('desertCamps.tag3', 'تجهيزات VIP')].map((tag, idx) => (
 ...
 {property.isPremium && <span className="bg-[#47CCD0] text-white text-xs font-bold px-3 py-1 rounded-md">★ {t('citySale.featured', 'مميز')}</span>}
 </div>
 ...
 <button className="px-4 py-2 bg-[#47CCD0]/10 text-[#47CCD0] rounded-lg text-sm font-bold hover:bg-[#47CCD0] hover:text-white transition-colors">{t('dailyRent.detailsAndBooking', 'التفاصيل والحجز')}</button>
 </div>
 </div>
 </div>
 ))}
